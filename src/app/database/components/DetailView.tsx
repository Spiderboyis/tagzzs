...
export default function DetailView({
  ...
  const getCardPreviewUrl = (url: string) => {
    if (!url) return "";
    ...
    <>
      <div className="flex flex-col h-full p-0 overflow-y-auto db-scroll fade-in">
        <div className="w-full max-w-5xl mx-auto flex flex-col gap-8 pt-4 px-8 pb-32">
          ...
          >
            {(() => {
              const thumbnail = currentDetailItem._original?.thumbnailUrl;
              const contentUrl =
                currentDetailItem._original?.link ||
                currentDetailItem._original?.url ||
                currentDetailItem._original?.sourceUrl;

              // 1. Thumbnail exists -> Use Thumbnail
              if (thumbnail) {
                return (
                  <img
                    src={thumbnail}
              ...
              // 2. No Thumbnail -> Check Content URL
              if (contentUrl) {
                ...
                // 2b. Is PDF/Web/Doc -> Render Iframe
                return (
                  <iframe
                    src={getCardPreviewUrl(contentUrl)}
              ...
              // 3. Fallback to default image prop
              return (
                <img
            ...
            <div className="absolute bottom-6 left-8 right-8">
              <div className="flex flex-wrap gap-2 mb-2">
                {(
                  currentDetailItem.tags || [
                    { id: "gen", name: currentDetailItem.category },
                  ]
                ).map((tag: any) => (
                  <div
                    key={tag.id}
          ...
          <a
            ...
            onClick={(e) => {
              if (
                ...
                toast({
                  title: "No Source URL",
                  description: "This unique item has no source URL.",
                  className: "bg-zinc-950 border-zinc-800 text-white",
                });
              }
            }}
            className="group flex items-center justify-between p-4 bg-[#0a0a0a] rounded-xl border border-white/5 hover:border-white/20 transition-all duration-300 hover:shadow-[0_0_20px_rgba(0,0,0,0.5)] cursor-pointer"
          >
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center group-hover:bg-white group-hover:text-black transition-colors border border-white/5">
          ...
          <div className="grid grid-cols-4 gap-4 h-24">
            ...
            >
              <FileText
                weight="bold"
                className={`text-2xl ${
                  isNotesOpen ? "text-white" : "text-zinc-600"
                } group-hover:text-white transition-colors`}
              />
              <span
                className={`text-xs font-bold ${
                  isNotesOpen ? "text-white" : "text-zinc-500"
            ...
            >
              ...
              />
              <span className="text-xs font-bold text-zinc-500 group-hover:text-white">
                Flashcards
              </span>
            </button>
            <button
      ...
      >
        <AlertDialogContent className="bg-zinc-950 border-zinc-800">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to remove the tag "{tagToDelete?.name}" from
              this content?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
