...
function DatabasePageContent() {
  ...
  const [kanbanExpandedCats, setKanbanExpandedCats] = useState<Set<string>>(
    new Set()
  );
  const [kanbanExpandedSubs, setKanbanExpandedSubs] = useState<Set<string>>(
    new Set()
  );
  ...
  const renderGraph = useCallback(() => {
    ...
    if (state === "category") {
      ...
      if (cat)
        l2Nodes = cat.children.map((s) => ({ label: s.name, items: s.items }));
    }
