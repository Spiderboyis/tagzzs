...
class ChromaCloudStorage:
    ...
    async def store_image_embedding(
        ...
        """
        Store image embedding in Chroma Cloud.

        Stores the CLIP embedding for the image in the 'images' collection.
        ...
        Args:
            ...
            user_id: User ID for multi-tenancy
            tags: Optional tags
            description: Image description (generated by Vision model)

        Returns:
            Document ID of stored image embedding

        Raises:
            Exception: If storage fails
        """
        try:
            doc_id = f"user_{user_id}_content_{content_id}_image"

            self.logger.info(
                f"🖼️ Storing image embedding for content_id={content_id}, user_id={user_id}"
            )
            ...
            metadata = {
                ...
                "source_type": "image",
                "tags": ",".join(tags) if tags else "",
                "description": description[:1000],  # Truncate if too long, primarily for debug/context
            }

            self.collection.add(
                ids=[doc_id],
                documents=[description],  # Store description as document text for hybrid search potential
                embeddings=[embedding],
                metadatas=[metadata],
            )

            self.logger.info(
                f"✅ Stored image embedding in Chroma Cloud collection: {self.collection.name}, doc_id={doc_id}"
            )

            return doc_id

        except Exception as e:
            self.logger.error(f"Failed to store image embedding: {str(e)}")
            raise
